...
onMounted(() => {
    useGetDataPinia.getData('http://127.0.0.1:8000/api/staff');
...
<template>
    <main class="container mx-auto mt-10 p-4">
        ...
        <section v-else class="text-center">
            <h2 class="text-3xl font-bold mb-10">¡Bienvenidos al apartado de profesores!</h2>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 justify-items-center">
                <div v-for="{ name, image, house, id } in useGetDataPinia.data?.data" :key="id"
                    ...
                    <CardStudent :name="name" :img="image" :house="house" :id="id" ruta="staff" />
                </div>
            </div>
